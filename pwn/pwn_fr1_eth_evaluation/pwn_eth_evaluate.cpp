...
void setInputParameters(SphericalPointProjector &pointProjector, 
...
int main(int argc, char ** argv) {
  ...
  // Usage
  if(argc < 5) {
    std::cout << "Usage: pwn_eth_evaluate <configuration.txt> <associations.txt> <odometry.txt> <reduceToICP> <save_pwn> <omegan_scale>" << std::endl
	      ...
	      << "\treduceToICP\t\t-->\tset to 1 if you want to reduce to GICP" << std::endl
	      << "\tsave_pwn\t\t-->\tset to 1 if you want to save pwn clouds" << std::endl
	      << "\tomegan_scale\t\t-->\tset to 1 if you want to reduce to GICP" << std::endl;
    return 0;
  }
  bool demoteToGICP = atoi(argv[4]);
  if(demoteToGICP) { std::cerr << "Demote to GICP" << std::endl; }
  bool save_pwn = false;
  float scale = 1.0f;

  if(argc > 5) {
    save_pwn = atoi(argv[5]);
    if(save_pwn) { std::cerr << "Saving clouds" << std::endl; }
  }
  if(argc > 6) {
    scale = atof(argv[6]);
  }
   ...
   *********************************************************************************/
  SphericalPointProjector pointProjector;
  ...
  linearizer.setScale(scale);
  if(demoteToGICP) {
    correspondenceFinder.setDemotedToGICP(true);
    linearizer.setDemotedToGeneralizedICP(true);
  }

  std::cerr << "sc: " << statsCalculator.minImageRadius() << " " << statsCalculator.maxImageRadius() << " " 
	    << statsCalculator.minPoints() << " " << statsCalculator.worldRadius() << " "
	    << statsCalculator.curvatureThreshold() << std::endl;
  std::cerr << "im: " << pointInformationMatrixCalculator.curvatureThreshold() << " " << normalInformationMatrixCalculator.curvatureThreshold() << std::endl;
  std::cerr << "cf: " << correspondenceFinder.imageRows() << " " << correspondenceFinder.imageCols() << " " 
	    << correspondenceFinder.inlierDistanceThreshold() << " " << correspondenceFinder.inlierNormalAngularThreshold() << " "
	    << correspondenceFinder.inlierCurvatureRatioThreshold() << " " << correspondenceFinder.flatCurvatureThreshold() << std::endl;
  std::cerr << "al: " << aligner.innerIterations() << " " << aligner.outerIterations() << " " 
	    << aligner.minInliers() << " " << linearizer.inlierMaxChi2() << std::endl;
  ...
  while(is.good()) {
    ...
    if(firstAssociation) {
      ...
      if(save_pwn) { refCloud->save((depthFilename + ".pwn").c_str(), globalT, 1, true); }
	...
	aligner.setReferenceCloud(refCloud);
	 ...
	 << globalRotation.x() << " " << globalRotation.y() << " " << globalRotation.z() << " " << globalRotation.w() 
	 << std::endl;
      if(save_pwn) { curCloud->save((depthFilename + ".pwn").c_str(), globalT, 1, true); }
...
void setInputParameters(SphericalPointProjector &pointProjector, 
  ...
  int cols = 640;
  int imageScale = 1;
  if((it = inputParameters.find("horizontalFov")) != inputParameters.end()) pointProjector.setHorizontalFov((*it).second);
  if((it = inputParameters.find("verticalFov")) != inputParameters.end()) pointProjector.setVerticalFov((*it).second);
  ...
  if((it = inputParameters.find("normalInformationMatrixScale")) != inputParameters.end()) normalInformationMatrixCalculator.setScale((*it).second);    
