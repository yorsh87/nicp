...
int main(int argc, char** argv) {
  ...
  if(argc == 2) {
    if(std::string(argv[1]) == "-help" || std::string(argv[1]) == "--help" || 
      ...
      std::cout << "USAGE: ";
      std::cout << "pwn_simple_viewer <working_directory> <apply_global_transform> <step> <point_size> <normal_length>" << std::endl;
      std::cout << "   <working_directory> \t-->\t directory where to find the .pwn files (default: .)" << std::endl;
  ...
  mainWindow->setWindowTitle("pwn_simple_viewer");
