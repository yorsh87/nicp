...
int main(int argc, char **argv) {
  ...
  mainWindow->setWindowTitle("pwn_cloud_prop_viewer");
  ...
  PWNQGLViewer *pwn_qglviewer = new PWNQGLViewer(mainWindow);
  viewer_layout->addWidget(pwn_qglviewer);
  pwn_qglviewer->init();
  pwn_qglviewer->setAxisIsDrawn(true);
  ...
  pwn_qglviewer->addDrawable(&pointsDrawable);
  pwn_qglviewer->addDrawable(&normalsDrawable);
  pwn_qglviewer->addDrawable(&covariancesDrawable);
  pwn_qglviewer->addDrawable(&uncertaintyDrawable);
  ...
  while(mainWindow->isVisible()) {
    ...
    if(snapshotButton->isDown()) {
      ...
      pwn_qglviewer->setSnapshotFormat(QString("PNG"));
      pwn_qglviewer->setSnapshotQuality(100);
      pwn_qglviewer->saveSnapshot(QString(snapshotName), true); 
      usleep(100000);
    }
    
    if(update)
      pwn_qglviewer->updateGL();    
