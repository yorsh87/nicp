...
class PWNTrackerApp {
public:
  PWNTrackerApp(const std::string& configurationFile, PWNTrackerAppViewer* viewer_) { 
    _viewer = viewer_;
...
protected:
  ...
  Eigen::Isometry3f _offsetT, _previousEstimateT;
...
int main(int argc, char** argv) {
   ...
   *********************************************************************************/
  if(argc < 7) {
    std::cout << "Usage: nicp_eth_kinect_estimate_visualizer <configuration.txt> <associations.txt> <odometry.txt> <groundtruth.txt> <algorithm> <save_snapshots>" << std::endl
	      ...
	      << "\t\t\t\t\tgicp\tdvo\tndt\tnicp\tkinfu\tnicp+" << std::endl
  ...
  PWNTrackerAppViewer* viewer;
  if(save_snapshots) { viewer = new PWNTrackerAppViewer(mainWindow, algorithm); }
  else { viewer = new PWNTrackerAppViewer(mainWindow); }
  ...
  while(mainWindow->isVisible()) {        
    ...
    while(is_ass.good() && (viewer->spinOnce() || viewer->spin())) {
      ...
      if(counter == 0) { 
	offsetT = groundtruthT;
	tracker->setStartingPose(groundtruthT);
      }  
