...
namespace nicp {
  ...
  void AlignerProjective::align() {
    ...
    CorrespondenceFinderProjective* cfp = dynamic_cast<CorrespondenceFinderProjective*>(_correspondenceFinder);
    assert(cfp && "AlignerProjective: _correspondenceFinder is not of type CorrespondnceFinderProjective");
    ...
    _projector->project(cfp->currentIndexImage(),
			cfp->currentDepthImage(),
			_currentCloud->points());

    if (_currentCloud->rgbs().size())
      _currentCloud->projectRGB(cfp->currentRGBImage(),
				cfp->currentIndexImage());
    _T = _initialGuess;
    for(int i = 0; i < _outerIterations; i++) {
      ...
      _projector->project(cfp->referenceIndexImage(),
			  cfp->referenceDepthImage(),
