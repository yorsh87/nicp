...
namespace nicp {
  ...
  void ParallelCylindricalPointProjector::unProject(PointVector &points, 
					  IntImage &indexImage,
					  const DepthImage &depthImage) const {
    if(_imageRows == 0 ||  _imageCols == 0) { 
      throw "ParallelCylindricalPointProjector: Depth image has zero dimensions";
    }
  ...
  void ParallelCylindricalPointProjector::projectIntervals(IntervalImage &intervalImage, 
