...
int main(int argc, char** argv) {
   ...
   if(argc < 5) {
      std::cout << "Usage: nicp_depth_camera_tracking <configuration.txt> <associations.txt> <odometry.txt> <enable_viewer> <groundtruth.txt>" << std::endl
		<< "tconfiguration.txtt-->tinput text configuration filename" << std::endl
		<< "tassociations.txtt-->tfiel containing a set of depth images associations for alignment in the format: " << std::endl
		<< "ttttttimestampDepthImage depthImageFilename" << std::endl
		<< "todometry.txttt-->toutput text filename containing the computed visual odometry" << std::endl
		<< "tenable_viewertt-->t1 if you want to enable the viewer, 0 otherwise" << std::endl
		<< "tground_truth.txtt-->toptional parameter to show groundtruth trajectory" << std::endl;
      return 0;
   }
