...
int main(int argc, char** argv) {
  ...
  if(argc < 2) {
    std::cout << "Usage: nicp_pomerleau2munich <groundtruth.csv>" << std::endl
  ...
  while(is.good()) {
    ...
    if(cloud->points().size() > 0) {
      ...
      nicp::DepthImage_convert_32FC1_to_16UC1(rawDepth, depth);
      cv::imwrite("depth/" + filename.substr(0, filename.length() - 3) + "pgm", rawDepth);
      // cloud->save(std::string(filename.substr(0, filename.length() - 3) + "nicp").c_str());
